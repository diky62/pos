...
@section('title')
    Transaksi Penjualan Utang
@endsection

@section('breadcrumb')
    @parent
    <li class="active">Transaksi Penjualan Utang</li>
@endsection
@push('css')
<style>
    .tampil-bayar {
...
{{--                         </div> --}}
                ...
                <div class="col-lg-7">  
                    <div class="tampil-bayar bg-primary" id="rupiah">TOTAL Rp. {{ number_format( $itemCarts->sum(function ($item) { return $item->harga_jual_utang * $item->carts->jumlah;
                        }),0,".",".") }} -,
                    {{--  {{
                        }} --}}
                    </div>
                            ...
                            <tbody>
                                @foreach($itemCarts as $item)
                                    <tr>
                                        ...
                                        <td>Rp {{ $item->harga_jual_utang }}</td>
                                        <td>{{ $item->carts->jumlah }}</td>
                                        <td>Rp {{ $item->diskon }}</td>
                                        <td>Rp {{ $item->harga_jual_utang * $item->carts->jumlah }}</td>
                                        <td>
                ...
                <div class="col-lg-4">
                    <br>
                    <div class="card-body">
                    <form action="{{ route('transaksi_utang.store') }}" method="post" name="form_transaksi" id="form_transaksi">
                        @csrf
                            ...
                            <div class="col-lg-9    ">
                                <input type="text" name="total_harga" id="total_harga" class="form-control"  value="{{ 
                                       $itemCarts->sum(function ($item) {
                                            return $item->harga_jual_utang * $item->carts->jumlah;
                                        })
                        ...
                        <div class="form-group row">
                           <div class="col-lg-9    ">
                                <input type="hidden" name="jenis_pembayaran" id="jenis_pembayaran" class="form-control"  value="2" readonly>
                                <span class="help-block with-errors"></span>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-lg-9    ">
                                <input type="hidden" name="status" id="status" class="form-control"  value="2" readonly>
                                <span class="help-block with-errors"></span>
                            </div>
                        </div>
                        {{-- <div class="col-lg-12">  
